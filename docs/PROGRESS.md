...
## 2026-09-13（深夜）拉取远端 + 报告正文准确性修正

### 拉取与分叉处理

本地 09-12 的提交 `d6f5e0e`（报告样例标注清理 + DEMO.md 重写）**当时推送未成功**，
远端在此期间新增 3 个提交（LLM 真实接入 / V5 UI 收尾 + Word 可研报告 / 聊天交互合并），
**两条线分叉且改了同一批文件**。

处理方式：
1. 本地提交先存入分支 `backup-20260913-local`（未丢失）
2. 对齐远端 `ae4c4c0`
3. **比对后发现：远端已独立修掉了「样例数据」标注问题**（grep 无命中），
   故我的清理工作已冗余，不再重复；只保留远端未做的部分

### 本轮实际改动

1. **恢复 `docs/DEMO.md`**（远端未改动此文件，接管我的重写版），并按最新实况同步：
   - 新增 LLM 为**可选增强**的说明（配 Key 启用，未配自动降级为规则解析）
   - 新增 VPS 单端口 8080 部署方式与「只认 dist」的流程口径
   - 报告导出补充 **Word 可研格式**
   - 「禁止出现的内容」表中关于大模型的表述改为准确说法

2. **⚠️ 修正报告正文的实质性错误（本次最重要）**

   `mock/report.ts`（预览/PDF）与 `api/reportDocx.ts`（Word 可研报告）共 10 处表述不成立：

   | 问题 | 原表述 | 修正后 |
   |---|---|---|
   | 系统已不做网格化评价 | "网格精度 X 米；评价单元 N 个" | "候选池为控规工业用地图斑 N 个" |
   | **过度声称**（6 类约束无图层数据） | "本次分析共启用 N 项硬约束，**全部候选地块均已通过校验**" | "对**具备图层数据支撑**的硬约束执行一票否决…缺少图层数据的约束未纳入计算，已在结果说明中逐条列明" |
   | **过度声称** | "该地块**满足全部硬性约束**" | "该地块**已通过本期可用的**硬约束校验" |
   | 用模板约束总数误导 | "共启用 ${scenario.constraints.length} 项" | 改为按实际参与情况表述 |
   | 数据说明缺时点提醒 | 仅写"真实图层数据" | 增加"⚠️ 各图层时点尚在核实，补齐前不得用于对外发布或审批依据" |

   **为什么必须改**：这些文字会随送审报告打印出去。声称"满足全部硬性约束"而实际有
   6 类约束从未参与计算，属虚假陈述，直接触碰责任声明第 1、2 条。

3. **重新安装前端依赖**：远端新增 `docx` 依赖，本地缺失导致类型检查报 20+ 处
   "Cannot find module"，补装 22 个包后恢复。

**当前状态**
- `vue-tsc --noEmit` **0 错误**
- `vite build` 成功（**2345 模块**；`reportDocx` 独立分包 382KB 按需懒加载，不拖累首屏）
- 工作区干净，新增分支 `backup-20260913-local`（存有已废弃的本地提交，可删）

**⚠️ 环境踩坑（重要）**

- **`git reset --hard` 在本机会静默删掉工作区文件**：本次执行后 21 个源文件
  （含 `src/types/ai.ts`、`store/*.ts`、`utils/*.ts` 等）从磁盘消失，但仍在 git 索引中，
  表现为类型检查爆出大量"找不到模块"。
  恢复方式：`git checkout -- $(git diff --name-only --diff-filter=D)`
  **执行 reset/checkout 后务必 `git status` 复查，不要直接开始类型检查。**
- 构建仍需**先单独 `rm -rf dist`** 再 `vite build`，不可串联。

**下一步（09-14，功能冻结日）**
1. **把新 `dist` 拷到 VPS**（改前端 = 本机构建 → 拷 dist → 硬刷新）
2. 逐层确认数据时点（18 图层）← 卡用户
3. 单位盖章流程 ← 卡用户，**最紧急**
4. 按 DEMO.md 录制视频
