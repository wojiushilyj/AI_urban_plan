...
LAYERS = [
    ...
    # ---- 市政设施（新增分类）----
    # 工业配套市政用地：供水/排水/供电/供燃气/通信/邮政/水工设施，81 图斑（源 82，剔 1 条空几何）。
    # 属工程性市政基础设施，与「服务与设施」（生产性服务业 + 文保）性质不同，单独成类。
    # 缺 .prj，按同批数据补 EPSG:4525（坐标范围校验通过：↔109.97–110.29°E / 25.06–25.31°N）。
    {
        "id": "municipal-land", "name": "工业配套市政用地", "shp": "工业配套市政用地",
        "group": "municipal",
        "keep": ["用地", "名称", "等级", "状态", "设施类", "规模", "备注"],
    },
    # ---- 现状建设（新增分类）----
    # 现状建筑分布：7851 个建筑轮廓面，含「区域」（苏桥/秧塘/乐和/两江/大圆盘）与「层数」（1–3 层）。
    # 建筑轮廓不是"用地"，与产业用地的口径不同，单独成类。
    {
        "id": "current-building", "name": "现状建筑分布", "shp": "现状建筑分布",
        "group": "built",
        "keep": ["区域", "层数"],
    },
]
